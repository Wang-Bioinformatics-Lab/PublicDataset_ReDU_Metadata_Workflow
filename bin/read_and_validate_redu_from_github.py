...
def complete_and_fill_REDU_table(df, allowedTerm_dict, add_usi = False, other_allowed_file_extensions = [], **kwargs):
    ...
    if 'UBERONBodyPartName' in df.columns:
        df['UBERONBodyPartName'] = df['UBERONBodyPartName'].astype(str)
    if 'DOIDCommonName' in df.columns:
        df['DOIDCommonName'] = df['DOIDCommonName'].astype(str)
